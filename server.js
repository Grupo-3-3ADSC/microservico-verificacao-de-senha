    ...
    const jti = uuidv4(); // id do token (para single-use)
    const payload = {
        sub: email,
    ...
    // Opcional: salve jti em storage temporário (Redis) com TTL = 15m para garantir one-time use.
    // exemplo simples em memória (NÃO para produção):
    usedResetTokens[jti] = { used: false, expires: Date.now() + 15 * 60 * 1000 };
